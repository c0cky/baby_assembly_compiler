...
sub process_user {
	...
	or die "Cannot open file $hw1_dir/comments.txt for writing ($!)\n";
	...
	if ($file eq 'ba' || $file eq 'lex.yy.c' || $file =~ /\.o$/) {
...
#    print(STDOUT "System msgs for $uname:\n");
    ...
    cmt("\nTesting ba with test files in $test_dir:\n\n");

#    $error_count = 0;

    for ($i = 1; (-e "$test_dir/test${i}.s"); $i++) {
	cmt("Running ba on test${i}.s; output sent to test${i}.out ...");
#	cmt("   ./ba $test_dir/test${i}.s < $test_dir/test${i}.in > test${i}.out 2> test${i}-err.txt");
	eval {
	    local $SIG{ALRM} = sub { die "TIMED OUT\n" };
	    alarm $timeout;
	    $rc = system("./ba $test_dir/test${i}.s < $test_dir/test${i}.in > test${i}.out 2> test${i}-err.txt");
	    alarm 0;
	};
	if ($@ && $@ eq "TIMED OUT\n") {
	...
	if (!(-e "test${i}.out")) {
	    cmt("  OUTPUT FILE test${i}.out DOES NOT EXIST (-10%)\n");
	    $score -= 10;
	    next;
	}

	# compare 
	cmt("  test${i}.out exists; comparing with sol'n file with \"diff\":\n");
	cmt("    diff test${i}.out $test_dir/test${i}.out\n");
	$report = `diff test${i}.out $test_dir/test${i}.out`;
	unlink "test${i}.out";
	# chomp $report;
	if ($report eq '') {
...
# 	cmt("\n");
#     }
    cmt("Your current score is $score percent, not counting error reporting.\n");
    cmt("Your error reporting will be checked manually.\n");
    cmt("######################################################\n");
}
